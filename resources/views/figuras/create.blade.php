...
<div class="row">
    <div class="col s8 offset-s2">
      <div class="card">
        <div class="card-content">
            <form id="formulario_envio" action="{{ route('figuras.store') }}" method='POST'>
                @csrf
                <div class="row">
                    <div class="input-field col s6">
                        <input required id="nombre" name="nombre" autofocus type="text" class="validate validacion_manual">
                        <label for="nombre">Nombre de la nueva figura geométrica</label>
                    </div>
                    <div class="input-field col s2">
                        <select  id="tipo" name="tipo" onchange="seleccion_subtipo()" class="validacion_manual">
                    ...
                    <div class="input-field col s2 ">
                        <select required id="subtipo" name="subtipo" class="validacion_manual">
                            
                        </select>
                        <label for="subtipo">Subtipo</label>
                    </div>
                    <div class="input-field col s2">
                        <select required id="color" name="color" class="validacion_manual">
                            <option value="" disabled selected>Seleccione</option>
                    ...
                    <div class="input-field col s10 offset-s1">
                        <textarea required id="descripcion" name="descripcion" class="validacion_manual materialize-textarea"></textarea>
                ...
                <div class="row">
                    <div  class="input-field col s2 offset-s5">
                        <input required  id="perimetro" readonly name="perimetro"  type="text" class="validate ">
  ...
  function validar_triangulo_equilatero(){
    return true;
  }
  ...
  function seleccion_subtipo(){
    calcular_numero_lados();
      $.post('getRequest',"seleccion_subtipo=1&tipo="+$("#tipo").val(),function(data){
          if (data.length!=0) {
  ...
  function calcular_numero_lados(){
    $.post('calcular_numero_lados',"nombre_tipo="+$("#tipo").val(),function(data){
          if (data.length!=0) {
            ...
            for (let index = 0; index <= (parseInt(data[0].lados)-1); index++) {
               ...
               if(data[0].lados==4){
                ...
                $("#contenedor_lados").append(' <div  class="input-field col s'+columnas+'"><input required id="lado'+(index+1)+'" name="lado'+(index+1)+'"  type="number" class="validate  lado validacion_manual"><label for="lado'+(index+1)+'">Lado '+(index+1)+'</label></div>');
                M.updateTextFields();
            }
            $('.lado').blur(()=>{
